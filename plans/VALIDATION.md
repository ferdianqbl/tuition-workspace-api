...
| | Cookie & Header Extraction | [auth.middleware.ts](https://github.com/ferdianqbl/tuition-workspace-api/blob/main/src/middlewares/auth.middleware.ts) | Resolves token validation from Bearer header (for general APIs) or secure cookies (for browser file download streams). | `[x] OK` |
...
| **B. Tuition Cases** | Case CRUD & Filters | [case.service.ts](https://github.com/ferdianqbl/tuition-workspace-api/blob/main/src/modules/case/case.service.ts) | Creates, reads, updates, and lists cases. Enforces parameter filters (subject, level, status) and keyword search mapping. | `[x] OK` |
| | Owner/Invited Row-Level Security | [case.repository.ts](https://github.com/ferdianqbl/tuition-workspace-api/blob/main/src/modules/case/case.repository.ts) | Attaches database filters ensuring Parents only query cases they own and Tutors only query cases they are invited to. | `[x] OK` |
...
| | Parent directory browse | [tutor.repository.ts](https://github.com/ferdianqbl/tuition-workspace-api/blob/main/src/modules/tutor/tutor.repository.ts) | Supports paginated searches and name query filters across public profiles for parent directory exploration. | `[x] OK` |
